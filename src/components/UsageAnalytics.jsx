...
const UsageAnalytics = () => {
  const [activePeriod, setActivePeriod] = useState('month'); // 'month', '30days', 'lifetime'
  ...
  // Helper function - define BEFORE usage
  ...
  }, [activePeriod, currentYear, currentMonth]);
  ...
  const highestApp = pieData.length > 0 ? pieData[0] : { name: 'None', minutes: 0 };
  const totalMinutes = pieData.reduce((sum, item) => sum + item.minutes, 0);
    ...
    <div className="w-full px-4 sm:px-0">
      ...
      <div className="flex justify-center mb-6">
        <div className="inline-flex p-1 bg-slate-100 rounded-2xl">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${activePeriod === 'month' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            📅 Month
          </button>
          <button
            onClick={() => setActivePeriod('30days')}
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${activePeriod === '30days' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            📊 Last 30 Days
          </button>
          <button
            onClick={() => setActivePeriod('lifetime')}
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${activePeriod === 'lifetime' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            🌍 Lifetime
          </button>
        </div>
      ...
      {activePeriod === 'month' && (
        <div className="flex justify-between items-center mb-6 sm:mb-8">
          <button onClick={() => changeMonth(-1)} className="p-2 sm:p-3 hover:bg-slate-100 rounded-full transition-colors" aria-label="Previous month">
          ...
          <button onClick={() => changeMonth(1)} className="p-2 sm:p-3 hover:bg-slate-100 rounded-full transition-colors" aria-label="Next month">
            <svg className="w-5 h-5 sm:w-6 sm:h-6 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-slate-400 bg-white rounded-3xl p-8">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-3xl p-4 sm:p-6 text-white shadow-lg text-center sm:text-left">
            ...
            <p className="text-xs opacity-80 mt-2">
              {pieData.length} app{pieData.length !== 1 ? 's' : ''} total
            </p>
          </div>
              ...
              <div className="w-full h-[280px] sm:h-[320px] md:h-[350px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip formatter={(value) => formatMinutes(value)} contentStyle={{ backgroundColor: '#fff', borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} />
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-4 sm:p-6 flex flex-col justify-center items-center text-center">
              ...
              {activePeriod === 'month' && totalDeviceTime > 0 && (
                <p className="text-xs text-slate-400 mt-2">
                  Device total: {formatMinutes(totalDeviceTime)}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-4 sm:p-6">
            <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
              <span>🏆</span> Top Apps by Usage
            </h3>
            <div className="space-y-3">
              {pieData.slice(0, 5).map((app, idx) => (
                <div key={app.name} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                  <div className="flex items-center gap-3">
                    <span className="text-lg font-bold text-indigo-600 w-6">#{idx + 1}</span>
                    <span className="font-medium text-slate-800">{app.name}</span>
                  </div>
                  <div className="text-sm text-slate-500 font-bold">{formatMinutes(app.minutes)}</div>
                </div>
              ))}
            </div>
          </div>
