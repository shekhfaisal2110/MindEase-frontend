...
const NotificationPanel = React.memo(({ onClose }) => {
  ...
  // Fetch notifications using cursor (if backend uses that) – adjust as needed
  const fetchNotifications = useCallback(async (reset = true) => {
    ...
    try {
      const url = cursor && !reset ? `/notifications?cursor=${cursor}&limit=15` : '/notifications?limit=15';
      ...
      } else if (data.notifications && data.pagination) {
  ...
  }, [hasMore, isLoadingMore, cursor, fetchNotifications]);
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-2xl max-h-[90vh] sm:rounded-3xl shadow-2xl flex flex-col sm:mx-4 overflow-hidden animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-300">
        ...
        <div className="flex justify-between items-center p-4 sm:p-5 border-b sticky top-0 bg-white z-10">
          ...
          <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100 active:bg-slate-200 transition touch-manipulation" aria-label="Close">
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
          ...
          {!loading && notifications.map(notif => {
            ...
            return (
              <div
                ...
                className={`p-3 sm:p-4 rounded-2xl border ${styles.border} ${!notif.isRead ? 'bg-white shadow-md' : styles.bg} transition-all duration-200 cursor-pointer hover:shadow-md active:scale-98`}
              >
                <div className="flex flex-col sm:flex-row justify-between items-start gap-2">
                  <div className="flex-1 min-w-0">
          ...
          {isLoadingMore && <div className="text-center py-2 text-slate-400 text-xs">Loading more...</div>}
        </div>
      </div>
    </div>
  );
