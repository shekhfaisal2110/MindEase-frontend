...
// src/pages/TherapyExercises.jsx
...
// Lazy load TherapyCalendar (if it's heavy, split it)
const TherapyCalendar = lazy(() => import('../components/TherapyCalendar'));

// Skeleton components – instant visual feedback
const ProgressSkeleton = () => (
  <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 mb-8 animate-pulse">
    <div className="flex flex-col sm:flex-row justify-between gap-4">
      <div><div className="h-6 bg-slate-200 rounded w-40 mb-2" /><div className="h-4 bg-slate-200 rounded w-48" /></div>
      <div className="w-32 h-10 bg-slate-200 rounded-xl" />
    </div>
    <div className="mt-4 flex justify-between"><div className="h-7 bg-slate-200 rounded w-16" /><div className="h-4 bg-slate-200 rounded w-20" /></div>
    <div className="mt-2 h-3 bg-slate-200 rounded-full" />
  </div>
);

const HotPotatoSkeleton = () => (
  <div className="relative mb-12 animate-pulse">
    <div className="bg-amber-50 rounded-[2.5rem] p-6 md:p-8 border border-amber-100">
      <div className="flex items-center gap-3 mb-4"><div className="w-7 h-7 bg-amber-200 rounded" /><div className="h-6 bg-amber-200 rounded w-40" /></div>
      <div className="h-4 bg-amber-200 rounded w-3/4 mb-6" />
      <div className="grid sm:grid-cols-3 gap-3">{[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-white/60 rounded-2xl" />)}</div>
    </div>
  </div>
);

const FormSkeleton = () => (
  <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-6 md:p-8 mb-12 animate-pulse">
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2"><div className="h-10 bg-slate-200 rounded-xl" /><div className="h-10 bg-slate-200 rounded-xl" /><div className="h-10 bg-slate-200 rounded-xl" /><div className="h-10 bg-slate-200 rounded-xl" /></div>
  ...
  <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 animate-pulse">
    <div className="flex flex-col sm:flex-row justify-between gap-3">
      <div className="flex-1 space-y-2">
        <div className="flex gap-2"><div className="h-5 w-16 bg-slate-200 rounded" /><div className="h-5 w-20 bg-slate-200 rounded" /></div>
        <div className="h-5 bg-slate-200 rounded w-3/4" />
      </div>
      <div className="flex items-center gap-3 shrink-0"><div className="w-20 h-10 bg-slate-200 rounded-full" /><div className="w-10 h-10 bg-slate-200 rounded-2xl" /><div className="w-10 h-10 bg-slate-200 rounded-2xl" /></div>
    </div>
  </div>
);
...
const ExercisesListSkeleton = () => (
  <div className="space-y-6">
    <div className="flex justify-between"><div className="h-5 bg-slate-200 rounded w-32 ml-2" /><div className="h-6 bg-slate-200 rounded w-20" /></div>
    <div className="grid gap-4">{[...Array(3)].map((_, i) => <ExerciseItemSkeleton key={i} />)}</div>
  </div>
);

const TherapyExercises = React.memo(() => {
  const [exercises, setExercises] = useState([]);
  const [type, setType] = useState('hotpotato');
  const [content, setContent] = useState('');
  const [loading, setLoading] = useState(true);
  ...
  // Record page view and fetch initial data
  useEffect(() => {
    const recordPageView = async () => {
      try {
        const res = await api.post('/daily-activity/page-view', { pageName: 'therapy' });
    ...
    recordPageView();
    Promise.all([fetchExercises(), fetchStats()]);
  }, []);
  ...
  const addExercise = useCallback(async (e) => {
    e.preventDefault();
    if (!content.trim()) return;
    try {
      const res = await api.post('/therapy', { type, content });
  ...
  const incrementRep = useCallback(async (id) => {
    const exercise = exercises.find(ex => ex._id === id);
    if (!exercise || exercise.count >= targetCount) return;
    try {
      const res = await api.post(`/therapy/increment/${id}`);
      const updated = res.data;
      setExercises(prev => prev.map(ex => ex._id === id ? updated : ex));
      // After increment, check if all exercises reached target
      const newExercises = exercises.map(ex => ex._id === id ? updated : ex);
      const allCompleted = newExercises.length > 0 && newExercises.every(ex => ex.count >= targetCount);
      if (allCompleted && !confettiTriggered) {
  ...
  }, [exercises, targetCount, refreshStats, confettiTriggered]);
  ...
  const getCreationDate = useCallback((exercise) => {
    const date = exercise.date || exercise.createdAt;
    if (!date) return null;
    return new Date(date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
  }, []);
  ...
  const completedExercises = useMemo(() => exercises.filter(ex => ex.count >= targetCount).length, [exercises, targetCount]);
  const totalExercises = exercises.length;
  const progressPercent = totalExercises === 0 ? 0 : (completedExercises / totalExercises) * 100;
  const allCompleted = totalExercises > 0 && completedExercises === totalExercises;
        ...
        <div className="container mx-auto px-4 py-10 max-w-3xl">
          ...
          <div className="flex justify-center mb-8">
            <div className="inline-flex p-1 bg-slate-100 rounded-2xl animate-pulse">
              <div className="w-24 h-10 bg-slate-200 rounded-xl" /><div className="w-24 h-10 bg-slate-200 rounded-xl ml-1" />
            </div>
          </div>
          <HotPotatoSkeleton />
          <FormSkeleton />
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] text-slate-800 pb-12">
      ...
      <div className="container mx-auto px-4 py-6 sm:py-10 max-w-3xl">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 p-5 sm:p-6 mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h3 className="text-base sm:text-lg font-bold text-slate-800">Therapy Challenge</h3>
              <p className="text-xs sm:text-sm text-slate-500">Each exercise must be repeated {targetCount} times.</p>
            </div>
            <div className="flex items-center gap-3">
              <select
                value={targetPeriod}
                onChange={(e) => setTargetPeriod(e.target.value)}
                className="bg-slate-50 border-none rounded-xl px-3 py-1.5 sm:px-4 sm:py-2 text-sm font-medium focus:ring-2 focus:ring-indigo-100 outline-none touch-manipulation"
              >
                <option value="week">1 Week (55 reps each)</option>
                <option value="month">1 Month (33 reps each)</option>
                <option value="year">1 Year (11 reps each)</option>
          ...
          <div className="mt-4 flex justify-between items-center">
            <span className="text-xl sm:text-2xl font-black text-indigo-600">{completedExercises}</span>
            <span className="text-xs sm:text-sm text-slate-400">/ {totalExercises} exercises completed</span>
          </div>
          <div className="mt-2 w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
            <div className="bg-gradient-to-r from-indigo-500 to-purple-500 h-full transition-all duration-700" style={{ width: `${progressPercent}%` }} />
          </div>
          {allCompleted && (
            <div className="mt-4 text-center text-emerald-600 font-bold text-sm sm:text-base">
              🎉 Congratulations! You've completed all exercises for the {targetPeriod} challenge! 🎉
            </div>
          )}
        </div>
        ...
        {viewMode === 'list' ? (
          <>
            {/* Hot Potato Card – responsive */}
            <section className="mb-10 sm:mb-12 relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-amber-200 to-orange-200 rounded-[2rem] sm:rounded-[2.5rem] blur opacity-25 group-hover:opacity-40 transition duration-1000" />
              <div className="relative bg-amber-50 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-6 md:p-8 border border-amber-100 shadow-sm">
                <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                  <span className="text-xl sm:text-2xl">🔥</span>
                  <h2 className="text-lg sm:text-xl font-black text-amber-900">Hot Potato Exercise</h2>
                </div>
                <p className="text-amber-800 text-xs sm:text-sm mb-5 sm:mb-6 leading-relaxed">
                  Repeat these phrases as many times as you like. Each click counts as one repetition.
                </p>
                <div className="grid sm:grid-cols-3 gap-2 sm:gap-3">
                  {['I am responsible for everything', 'I am sorry, please forgive me', 'Thank you, I love you'].map((phrase, i) => (
                    <div key={i} className="bg-white/60 backdrop-blur-sm p-2 sm:p-3 rounded-xl sm:rounded-2xl text-[11px] sm:text-xs font-bold text-amber-900 border border-amber-200/50 shadow-sm">
                      "{phrase}"
                    </div>
                  ))}
                </div>
            ...
            <section className="bg-white rounded-2xl sm:rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-5 sm:p-6 md:p-8 border border-slate-100 mb-10 sm:mb-12">
              <form onSubmit={addExercise} className="space-y-4">
                <div>
                  <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 block">
                    Exercise Category
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {['hotpotato', 'forgiveness', 'selftalk', 'receiving'].map((t) => (
                      <button
                        ...
                        type="button"
                        onClick={() => setType(t)}
                        className={`py-1.5 sm:py-2 px-1 text-[9px] sm:text-[10px] font-black rounded-xl border-2 transition-all truncate uppercase tracking-tighter touch-manipulation ${
                          type === t
                            ? 'border-indigo-600 bg-indigo-50 text-indigo-700'
                            : 'border-slate-50 bg-slate-50 text-slate-400'
                        }`}
                      >
                        {t === 'hotpotato' ? 'Potato' : t.charAt(0).toUpperCase() + t.slice(1)}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <textarea
                  ...
                  value={content}
                  onChange={e => setContent(e.target.value)}
                  className="w-full bg-slate-50 border-none rounded-xl sm:rounded-2xl p-4 sm:p-5 text-slate-700 placeholder:text-slate-300 focus:ring-2 focus:ring-indigo-100 outline-none transition-all min-h-[100px] sm:min-h-[120px] shadow-inner resize-none"
                  required
                />

                <button type="submit" className="w-full bg-slate-900 hover:bg-indigo-600 text-white font-black py-3 sm:py-4 rounded-xl sm:rounded-2xl transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 touch-manipulation">
                  <span>Commit to Exercise</span>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" /></svg>
                </button>
              </form>
            ...
            <div className="space-y-5 sm:space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-xs sm:text-sm font-black text-slate-400 uppercase tracking-widest ml-2">Active Practice</h2>
                <span className="text-[10px] sm:text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full">
                  {uniqueDays} days active
                </span>
              </div>
              {exercises.length === 0 ? (
                <div className="text-center py-12 sm:py-20 bg-slate-50 rounded-2xl sm:rounded-[3rem] border-2 border-dashed border-slate-200">
                  <p className="text-slate-400 font-medium text-sm sm:text-base">Ready to start? Add your first exercise above.</p>
                </div>
              ) : (
                <div className="grid gap-3 sm:gap-4">
                  {exercises.map(ex => {
                    ...
                    const isCompleted = ex.count >= targetCount;
                    return (
                      <div key={ex._id} className="bg-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 group">
                        <div className="flex flex-col sm:flex-row justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center flex-wrap gap-2 mb-1">
                              <span className={`text-[8px] sm:text-[9px] font-black uppercase px-2 py-0.5 rounded-lg border tracking-wider ${getTypeStyles(ex.type)}`}>
                                {ex.type}
                              </span>
                              {creationDate && (
                                <span className="text-[8px] sm:text-[9px] text-slate-400 flex items-center gap-1">
                              ...
                              {isCompleted && (
                                <span className="text-[8px] sm:text-[9px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">
                                  Goal Met
                                </span>
                              )}
                            </div>
                            <p className="text-slate-700 font-medium leading-relaxed break-words text-sm sm:text-base">
                              {ex.content}
                            </p>
                          </div>
                          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                            <div className="flex items-center gap-1 bg-slate-50 rounded-full px-2 sm:px-3 py-1">
                              <span className="text-base sm:text-lg font-bold text-indigo-600">{ex.count}</span>
                              <span className="text-[10px] sm:text-xs text-slate-400">/ {targetCount}</span>
                            </div>
                            <button
                              onClick={() => incrementRep(ex._id)}
                              disabled={isCompleted}
                              className={`font-bold px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl transition flex items-center gap-1 touch-manipulation ${
                                isCompleted
                                  ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                  : 'bg-indigo-100 hover:bg-indigo-600 text-indigo-600 hover:text-white'
                              }`}
                            ...
                            <button
                              onClick={() => deleteExercise(ex._id)}
                              className="text-slate-300 hover:text-rose-500 transition p-1 touch-manipulation"
                              title="Delete exercise"
                            >
          ...
          <Suspense fallback={<div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 animate-pulse h-96" />}>
            <TherapyCalendar />
          </Suspense>
        )}
      </div>
...
TherapyExercises.displayName = 'TherapyExercises';
export default TherapyExercises;