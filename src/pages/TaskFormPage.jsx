...
const TaskForm = React.memo(({ onTaskAdded }) => {
  // Memoize static configuration to avoid recreation
  const initialValues = {
    title: '',
    description: '',
    priority: 'medium',
  ...
  const handleSubmit = useCallback(async (values, { resetForm }) => {
    try {
      ...
      // Add activity points (non‑blocking)
      try {
        await api.post('/activity/add', { actionType: 'dailyTask', points: 3 });
      } catch (activityErr) {
        console.error('Failed to add activity points:', activityErr);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 sm:px-0 py-4 sm:py-6">
      <div className="bg-white rounded-2xl sm:rounded-[2rem] shadow-xl shadow-slate-200/60 border border-slate-100 overflow-hidden">
        
        {/* Form Header – responsive spacing */}
        <div className="bg-slate-50 px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-9 h-9 sm:w-10 sm:h-10 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-100">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" />
              </svg>
            </div>
            <div>
              <h2 className="text-lg sm:text-xl font-black text-slate-800 tracking-tight">New Task</h2>
              <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Workspace / Personal</p>
            </div>
          </div>
        </div>

        {/* Formik Wrapper – responsive padding */}
        ...
        {/* Footer Hint */}
        <div className="bg-slate-50/50 px-6 sm:px-8 py-3 sm:py-4 border-t border-slate-50">
