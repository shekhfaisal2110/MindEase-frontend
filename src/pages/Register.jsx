...
// ----------------------------------------------------------------------
// RegistrationForm – self‑contained, memoized, with loading feedback
// ----------------------------------------------------------------------
const RegistrationForm = memo(({ onSubmit, serverError, loading }) => {
  const usernameRef = useRef(null);
  const emailRef = useRef(null);
  const passwordRef = useRef(null);
  const confirmPasswordRef = useRef(null);
    ...
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      <div>
        ...
        <input
          ...
          name="username"
          placeholder="e.g. wanderer_01"
          className="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm focus:ring-2 focus:ring-indigo-100 outline-none touch-manipulation"
        />
      </div>
      ...
      {emailExists && (
        <div className="p-3 bg-amber-50 rounded-xl text-sm animate-in fade-in duration-200">
          <p className="font-bold text-amber-700">⚠️ This email already has an account</p>
          {recentLogins.length > 0 && (
            <div className="mt-2">
              <p className="text-xs font-semibold text-amber-600">Recent logins:</p>
          ...
          <p className="text-xs text-amber-600 mt-2">
            <Link to="/forgot-password" className="underline touch-manipulation">Forgot password?</Link> or{' '}
            <Link to="/login" className="underline touch-manipulation">login here</Link>.
          </p>
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-slate-900 hover:bg-indigo-600 text-white font-black py-4 rounded-2xl transition-all shadow-xl active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
      ...
      <div className="pt-6 text-center">
        <p className="text-sm font-medium text-slate-500">
          Already have an account?{' '}
          <Link to="/login" className="text-indigo-600 font-black hover:underline underline-offset-4 touch-manipulation">
            Log in
          </Link>
        </p>
      </div>
...
const Register = () => {
  ...
  const AuthContainer = ({ children, title, subtitle }) => (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-50 via-indigo-50 to-purple-50 flex items-center justify-center p-4 sm:p-6">
      <div className="bg-white/90 backdrop-blur-md rounded-3xl sm:rounded-[2.5rem] shadow-2xl shadow-indigo-100 p-6 sm:p-8 md:p-12 max-w-md w-full border border-white relative overflow-hidden">
        {/* Progress bar */}
        ...
        <div className="mb-8 sm:mb-10 text-center">
          ...
          <p className="text-slate-500 text-sm sm:text-base font-medium mt-2">{subtitle}</p>
        </div>
        {children}
      </div>
    </div>
  ...
  if (step === 1) {
    return (
      <AuthContainer title="Join the Journey" subtitle="Create your account to get started.">
        <RegistrationForm onSubmit={handleRegister} serverError={error} loading={submitting} />
      </AuthContainer>
    );
  }
      ...
      <form onSubmit={handleVerify} className="space-y-6">
        <div className="relative group">
          ...
          <input
            ...
            autoFocus
            maxLength="6"
            className="w-full text-center text-3xl sm:text-4xl font-black tracking-[0.3em] sm:tracking-[0.4em] py-4 sm:py-5 bg-slate-50 border-none rounded-2xl focus:ring-4 focus:ring-indigo-100 outline-none transition-all text-indigo-700 touch-manipulation"
            placeholder="000000"
          />
        </div>
        {error && (
          <div className="p-4 bg-rose-50 border border-rose-100 text-rose-600 text-sm rounded-2xl flex items-start gap-2 animate-in fade-in">
        ...
        <div className="space-y-3">
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-slate-900 hover:bg-indigo-600 text-white font-black py-4 rounded-2xl transition-all shadow-xl active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 touch-manipulation"
          >
            {submitting ? (
              <>
                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
          ...
          <button
            ...
            className="w-full py-2 text-sm font-bold text-slate-400 hover:text-indigo-600 disabled:opacity-50 transition-colors touch-manipulation"
          >
            {isResending ? 'Sending code...' : "Didn't get the code? Resend"}
          </button>
        </div>
