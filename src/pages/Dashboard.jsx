...
const Dashboard = () => {
  ...
  useEffect(() => { fetchTasks(); fetchTodayRoutine(); }, []);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Left Column: Quick Stats & Self-Care */}
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
            ...
            <div className="flex space-x-2">
              <select 
                ...
                className="flex-1 bg-slate-50 border-none rounded-2xl p-3 text-sm focus:ring-2 focus:ring-indigo-100 outline-none"
              >
              ...
              <button 
                onClick={updateMood} 
                className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-6 py-3 rounded-2xl transition-all active:scale-95 text-sm"
              >
                Save
              </button>
            </div>
            {todayRoutine?.mood && (
              <div className="mt-4 flex items-center justify-between bg-indigo-50 p-3 rounded-xl">
                <span className="text-xs font-bold text-indigo-600 uppercase">Current Feeling</span>
                <span className="text-lg font-black text-indigo-700">{todayRoutine.mood}/10</span>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-slate-800 flex items-center">
                <span className="mr-2">✨</span> Rituals
              </h2>
              <span className="text-[10px] font-black bg-emerald-50 text-emerald-600 px-2 py-1 rounded-md">
                {todayRoutine?.items.filter(i=>i.completed).length || 0}/{todayRoutine?.items.length || 0}
              </span>
            </div>
            <div className="space-y-1">
              {todayRoutine?.items.map((item,idx)=>(
                <label key={idx} className="flex items-center space-x-3 p-2 hover:bg-slate-50 rounded-xl transition-colors cursor-pointer group">
                  ...
                  <span className={`text-sm font-medium ${item.completed ? 'line-through text-slate-300': 'text-slate-600'}`}>
                    {item.name}
                  </span>
                </label>
              ))}
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
            <div className="flex justify-between items-end mb-4">
              <div>
                <h2 className="text-lg font-bold text-slate-800">Task Momentum</h2>
                <p className="text-xs text-slate-400 font-medium">{completedTasks} of {tasks.length} objectives reached</p>
              </div>
            ...
            <form onSubmit={createTask} className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <select 
                  ...
                  className="flex-1 bg-slate-50 border-none rounded-2xl p-4 text-sm focus:ring-2 focus:ring-indigo-100 outline-none font-medium"
                >
                ...
                <input 
                  ...
                  className="flex-1 bg-slate-50 border-none rounded-2xl p-4 text-sm focus:ring-2 focus:ring-indigo-100 outline-none font-medium text-slate-500" 
                />
                <button 
                  type="submit" 
                  className="bg-slate-900 hover:bg-indigo-600 text-white font-bold px-8 py-4 rounded-2xl transition-all active:scale-95 shadow-lg shadow-slate-200"
                >
                  Add Task
                </button>
              </div>
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-3xl p-12 text-center border border-dashed border-slate-200">
                <p className="text-slate-400 font-medium italic">No tasks yet. Take it slow.</p>
              </div>
            ) : (
