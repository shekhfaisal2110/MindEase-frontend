...
const ProgressSkeleton = () => (
  <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 animate-pulse">
    <div className="flex justify-between items-end">
      <div><div className="h-4 bg-slate-200 rounded w-32 mb-2" /><div className="h-8 bg-slate-200 rounded w-20" /></div>
...
const ActivitySkeleton = () => (
  <div className="bg-white rounded-2xl p-4 border border-slate-50 shadow-sm animate-pulse flex items-center gap-4">
    <div className="w-6 h-6 bg-slate-200 rounded-lg" />
    <div className="flex-1"><div className="h-5 bg-slate-200 rounded w-32 mb-1" /><div className="h-3 bg-slate-200 rounded w-20" /></div>
  </div>
);
...
const DailyTracker = React.memo(() => {
  ...
  const fetchAllTracks = useCallback(async () => {
    try {
      ...
      else if (res.data.tracks && Array.isArray(res.data.tracks)) tracks = res.data.tracks;
      const map = {};
      tracks.forEach(t => {
        const dateKey = t.date ? t.date.split('T')[0] : (t.dateStr || '');
        if (dateKey) map[dateKey] = t;
  ...
  const fetchDateTrack = useCallback(async (dateStr) => {
    setLoading(true);
    try {
      const res = await api.get(`/dailytrack/date/${dateStr}`);
      const trackData = res.data.track || res.data;
  ...
  useEffect(() => {
    ...
    fetchToday();
    fetchAllTracks();
  }, [fetchToday, fetchAllTracks]);

  useEffect(() => {
    if (viewMode === 'today') fetchDateTrack(selectedDate);
  }, [selectedDate, viewMode, fetchDateTrack]);

  const handleCheckboxChange = useCallback(async (key, checked) => {
    const wasCompleted = track[key];
    const taskName = activities.find(a => a.key === key)?.label || key;
    const shouldAddPoints = !wasCompleted && checked;
    ...
    if (allTasksCompleted && !confettiTriggered) {
      // Lazy load confetti only when first full completion occurs
    ...
    if (shouldAddPoints) {
      ...
      } catch (err) { console.error('Failed to add activity points:', err); }
    }
  }, [track, activities, confettiTriggered]);
  ...
  }, [selectedDate, track, fetchAllTracks]);
  ...
  const renderCalendar = useMemo(() => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        <button
          key={dateStr}
          onClick={() => { setSelectedDate(dateStr); setViewMode('today'); }}
          className={`relative aspect-square flex flex-col items-center justify-center rounded-2xl transition-all duration-300 touch-manipulation
            ${isSelected ? 'ring-2 ring-indigo-500 ring-offset-2' : ''}
            ${allDone ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white border border-slate-100 hover:border-indigo-200'}
          `}
        >
    ...
    return days;
  }, [currentYear, currentMonth, calendarData, activities, selectedDate]);

  if (loading && viewMode === 'today') {
    return (
      <PageLayout title="Daily Growth" subtitle="Track your 6 core habits for a balanced mind.">
        <div className="flex p-1 bg-slate-100 rounded-2xl mb-8 max-w-xs mx-auto">
          <div className="flex-1 py-2 text-center"><div className="h-5 bg-slate-200 rounded w-16 mx-auto" /></div>
          <div className="flex-1 py-2 text-center"><div className="h-5 bg-slate-200 rounded w-16 mx-auto" /></div>
        </div>
  ...
  return (
    <PageLayout title="Daily Growth" subtitle="Track your 6 core habits for a balanced mind.">
      ...
      <div className="flex p-1 bg-slate-100 rounded-2xl mb-8 max-w-xs mx-auto">
        <button
          onClick={() => setViewMode('today')}
          className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all touch-manipulation ${
            viewMode === 'today' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          Today
        </button>
      ...
      {viewMode === 'calendar' ? (
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 sm:p-6 animate-in fade-in zoom-in-95">
          <div className="flex justify-between items-center mb-6 sm:mb-8 flex-wrap gap-2">
            <button onClick={() => changeMonth(-1)} className="p-2 hover:bg-slate-50 rounded-lg active:scale-95 touch-manipulation" aria-label="Previous month">←</button>
            <h2 className="text-base sm:text-lg font-black text-slate-800">
              {new Date(currentYear, currentMonth).toLocaleString('default', { month: 'long', year: 'numeric' })}
            </h2>
            <div className="flex gap-2">
              <button onClick={goToday} className="px-3 py-1.5 text-xs font-medium bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 active:scale-95">Today</button>
              <button onClick={() => changeMonth(1)} className="p-2 hover:bg-slate-50 rounded-lg active:scale-95 touch-manipulation" aria-label="Next month">→</button>
            </div>
          </div>
          {/* FIXED: weekday headers with unique keys */}
          <div className="grid grid-cols-7 gap-1 sm:gap-2 text-center mb-4">
            {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(day => (
              <div key={day} className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest">{day}</div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-2 sm:gap-3">{renderCalendar}</div>
          <div className="mt-6 pt-4 sm:pt-6 border-t border-slate-50 flex items-center justify-center gap-4 flex-wrap">
            <div className="flex items-center text-xs text-slate-500"><div className="w-3 h-3 bg-indigo-600 rounded-sm mr-2" /> All Done</div>
            <div className="flex items-center text-xs text-slate-500"><div className="w-3 h-3 bg-indigo-100 rounded-sm mr-2" /> Partial</div>
          </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto space-y-4 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-slate-100 overflow-hidden relative">
            <div className="relative z-10 flex justify-between items-end">
              <div>
                <p className="text-[10px] sm:text-xs font-black text-indigo-500 uppercase tracking-wider mb-1">Today's Completion</p>
                <h3 className="text-2xl sm:text-3xl font-black text-slate-800">{completedCount}<span className="text-slate-300">/{activities.length}</span></h3>
              </div>
              <div className="text-right"><span className="text-sm sm:text-base font-bold text-slate-400">{Math.round(progressPercent)}%</span></div>
            </div>
            <div className="mt-4 w-full bg-slate-100 rounded-full h-3 overflow-hidden">
              <div className="bg-gradient-to-r from-indigo-500 to-purple-500 h-full transition-all duration-1000" style={{ width: `${progressPercent}%` }} />
            </div>
              ...
              <div key={act.key} className={`bg-white rounded-2xl border-2 transition-all duration-300 ${track[act.key] ? 'border-transparent bg-slate-50/50' : 'border-slate-50 hover:border-indigo-100 shadow-sm'}`}>
                <div className="p-4">
                  <label className="flex items-center cursor-pointer touch-manipulation">
                    <div className="relative flex items-center justify-center mr-4">
                      <input type="checkbox" checked={track[act.key]} onChange={e => handleCheckboxChange(act.key, e.target.checked)} className="peer sr-only" />
                      <div className="w-6 h-6 border-2 border-slate-200 rounded-lg transition-all peer-checked:bg-indigo-600 peer-checked:border-indigo-600 flex items-center justify-center">
                        <svg className={`w-4 h-4 text-white transition-transform ${track[act.key] ? 'scale-100' : 'scale-0'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                      </div>
                    </div>
                    <div className="flex-grow">
                      <span className={`font-bold block text-sm sm:text-base ${track[act.key] ? 'text-slate-400 line-through' : 'text-slate-800'}`}>{act.label}</span>
                      <span className="text-[9px] sm:text-[10px] font-bold text-indigo-400 uppercase tracking-widest">{act.target} goal</span>
                    </div>
                  </label>

                  {act.key === 'affirmationCompleted' && track.affirmationCompleted && (
                    <div className="mt-4 animate-in slide-in-from-top-2">
                      <textarea value={track.affirmationText} onChange={e => setTrack({ ...track, affirmationText: e.target.value })} placeholder="Speak it into existence..." rows="2" className="w-full p-3 sm:p-4 bg-white border border-indigo-100 rounded-xl text-sm focus:ring-2 focus:ring-indigo-100 outline-none resize-none" />
                    </div>
                  )}
                  {act.key === 'journalingCompleted' && track.journalingCompleted && (
                    <div className="mt-4 animate-in slide-in-from-top-2">
                      <textarea value={track.journalingText} onChange={e => setTrack({ ...track, journalingText: e.target.value })} placeholder="What's on your mind?" rows="4" className="w-full p-3 sm:p-4 bg-white border border-indigo-100 rounded-xl text-sm focus:ring-2 focus:ring-indigo-100 outline-none resize-none" />
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-100 shadow-sm">
            <h4 className="text-sm font-bold text-slate-800 mb-3 flex items-center"><span className="mr-2">📝</span> Daily Reflections</h4>
            <textarea value={track.notes} onChange={e => setTrack({ ...track, notes: e.target.value })} placeholder="Any other thoughts on today?" rows="3" className="w-full p-3 sm:p-4 bg-slate-50 border-none rounded-2xl text-sm focus:ring-2 focus:ring-indigo-100 outline-none resize-none" />
          </div>

          {/* Save button (sticky on mobile) */}
          <div className="sticky bottom-4 sm:bottom-6 left-0 right-0 py-3 px-2">
            <button onClick={saveTracking} disabled={saving} className={`w-full py-3 sm:py-4 rounded-2xl font-black text-white shadow-xl transition-all active:scale-95 flex items-center justify-center gap-2 touch-manipulation ${saving ? 'bg-slate-400' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200'}`}>
              {saving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <> <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg> <span>Sync Progress</span> </>}
            </button>
          </div>
        </div>
      )}
...
DailyTracker.displayName = 'DailyTracker';
export default DailyTracker;