...
// ------------------------------------------------------------
// Main component – no infinite loops
// ------------------------------------------------------------
const Analytics = React.memo(() => {
  ...
  const isFetchingRef = useRef(false);        // prevents concurrent calls
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const activities = historyRes.data.activities || historyRes.data;
      setHistory(activities);
      const total = activities.reduce((sum, day) => sum + (day.totalPoints || 0), 0);
      setLast30DaysTotal(total);
      setAllTimeTotal(allTimeRes.data.allTimeTotal || 0);
    } catch (err) {
  ...
  const breakdownTotals = React.useMemo(() => 
    history.reduce((acc, day) => {
      ...
      acc.pageView += b.pageView || 0;
      acc.hourlyEmotion += b.hourlyEmotion || 0;
      acc.emotionalCheckIn += b.emotionalCheckIn || 0;
      acc.gratitude += b.gratitude || 0;
      acc.affirmation += b.affirmation || 0;
      acc.growthHealing += b.growthHealing || 0;
      acc.letterToSelf += b.letterToSelf || 0;
      acc.dailyTask += b.dailyTask || 0;
      acc.reactResponse += b.reactResponse || 0;
      acc.ikigaiItem += b.ikigaiItem || 0;
      return acc;
    }, {
  ...
  const radarData = React.useMemo(() => 
    Object.entries(breakdownTotals).map(([name, value]) => ({
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex justify-between mb-6"><div className="h-8 bg-slate-200 rounded w-48" /><div className="h-10 bg-slate-200 rounded w-28" /></div>
          <StatsSkeleton />
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-6 md:py-8 max-w-7xl">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <button onClick={handleRefresh} disabled={refreshing} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 transition disabled:opacity-50 flex items-center space-x-2 shadow-sm touch-manipulation">
            {refreshing ? (
              <>
        ...
        {/* Stats cards, charts – same as before, omitted for brevity */}
